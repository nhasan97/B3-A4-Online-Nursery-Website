...
import { MdDelete, MdEditDocument, MdInfo } from "react-icons/md";
import useBlogCrudContext from "@/hooks/useBlogCrudContext";

const BlogManagementCard = ({ blog }: { blog: Tblog }) => {
  const { _id, title, thumbnail, author, authorImage, authorEmail, createdAt } =
    blog;

  const { handleDeleteBlog } = useBlogCrudContext() as TBlogCrudContext;
  ...
  return (
    <div className="flex flex-col xl:flex-row items-center mt-12 xl:ml-12 xl:mt-0 relative">
      ...
      <div className="w-full flex flex-col gap-2 px-5 pt-32 pb-5 xl:py-10 xl:pr-5 xl:pl-[170px] bg-white text-[#757575] rounded-lg">
        <div>
        ...
        <div className="flex flex-wrap sm:flex-nowrap items-center gap-3">
          <Link to={`/admin-dashboard/blog-details/${_id}`}>
            <Button className="w-fit bg-[#5D7E5F] text-base px-8 mt-3 rounded-full">
              <MdInfo />
            </Button>
          </Link>
          <Link to={`/admin-dashboard/edit-blogs/${_id}`}>
            <Button className="w-fit bg-[#5D7E5F] text-base px-8 mt-3 rounded-full">
              <MdEditDocument />
            </Button>
          </Link>
          <Button
            className="w-fit bg-[#5D7E5F] text-base px-8 mt-3 rounded-full"
